...
<div>
  {#each Stories[edition].stories as story}
    <div>
      <a class="underline" href="/short-stories/read/{$page.params.date}/{story.title}">{story.title}</a>
      by {story.author}
    </div>
  {/each}
</div>
...
<button type="button" class="btn variant-filled-primary">  <!-- TODO pdf download -->
